...
def report(rows, **kwargs):
    args=dict(epoch_id='bank',cutover=CUTOVER,start_date=date(2026,8,21),
              end_date=date(2026,8,24),as_of=ASOF)
    args.update(kwargs)
...
@pytest.mark.parametrize('change',[
    ...
    {'payload_sha256':'0'*64}, {'local_date':date(2026,8,22)},
])
...
def test_unfinished_day_refused_even_if_computed_before_asof():
    ...
    with pytest.raises(ValueError,match='completed'):
        report([row],as_of=datetime(2026,8,21,12,tzinfo=timezone.utc))
...
def test_cli_power_routes_only_to_qualified_reader(monkeypatch,tmp_path,capsys):
    ...
    assert cli.main(['report','power','--start','2026-08-21','--end','2026-08-24',
...
def test_cli_requires_explicit_power_policy(capsys):
    assert cli.main(['report','power','--start','2026-08-21','--end','2026-08-24'])!=0
