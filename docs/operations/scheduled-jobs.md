...
for raw collection and deterministic electrical safety. These jobs read raw
history and write only `energy_analytics` or private files below
`~/.local/state/earthship-energy`. They never invoke Codex, write Hexmem,
change an OpenHAB Item, or control hardware.
